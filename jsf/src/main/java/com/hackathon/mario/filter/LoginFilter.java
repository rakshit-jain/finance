package com.hackathon.mario.filter;
...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
	        ...
	        String otpURL = request.getContextPath() + "/otp";
	        UserProfile user = null;
	        if(session!=null){
	        	 user = (UserProfile) session.getAttribute("userProfile");
	        }
	        boolean loggedIn = (session != null && user != null && user.getName() != null);
	        boolean loginRequest = request.getRequestURI().equals(loginURL);
	        boolean registerRequest = request.getRequestURI().equals(registerURL);
	        boolean otpRequest = request.getRequestURI().equals(otpURL);
	        boolean resourceRequest = request.getRequestURI().startsWith(request.getContextPath() + ResourceHandler.RESOURCE_IDENTIFIER + "/");
	        boolean ajaxRequest = "partial/ajax".equals(request.getHeader("Faces-Request"));

	        if (loggedIn || loginRequest || resourceRequest || registerRequest || otpRequest) {
	            if (!resourceRequest) { // Prevent browser from caching restricted resources. See also http://stackoverflow.com/q/4194207/157882
	                ...
	                if(user.getUserRole() != null){
		                for(UserRole  userRole : user.getUserRole()){
		                	if(UserRoleTypeEnum.ADMIN.equals(userRole.getRoleTypeEnum())){
		                		adminUser = true;
		                		break;
		                	}
		                }
	                }
	                if(request.getRequestURI().contains("admin") && !adminUser){
