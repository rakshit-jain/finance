package com.hackathon.webservice.gson;
...
public class GsonMessageBodyHandler<T> implements MessageBodyWriter<T>, MessageBodyReader<T> {
	...
	public ExclusionStrategy exculsionStrategy;

	public ExclusionStrategy getExculsionStrategy() {
		return exculsionStrategy;
	}

	public void setExculsionStrategy(ExclusionStrategy exculsionStrategy) {
		this.exculsionStrategy = exculsionStrategy;
	}
	...
	private GsonBuilder GSONBuilderForReading;
	private GsonBuilder GSONBuilderForWriting;

	private GsonBuilder getReadingGsonbuilder() {
		if (GSONBuilderForReading == null) {
			GsonBuilder gson = getBaseGsonBuilder();
			GSONBuilderForReading = gson;
		}
		return GSONBuilderForReading;
	}

	private GsonBuilder getGsonbuilder() {
		if (GSONBuilderForWriting == null) {
			GsonBuilder gson = getBaseGsonBuilder();
			gson.setExclusionStrategies(getExculsionStrategy());
			GSONBuilderForWriting=gson;
		}
		return GSONBuilderForWriting;
	}
	...
	public T readFrom(Class<T> type, Type genericType, Annotation[] annotations, MediaType mediaType,
		...
		return getReadingGsonbuilder().create().<T> fromJson(entityReader, targetType);
	}
